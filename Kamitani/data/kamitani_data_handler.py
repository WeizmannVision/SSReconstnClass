...
test_im = pd.read_csv(dir + 'images/imageID_test.csv', header=None)
train_im = pd.read_csv(dir + 'images/imageID_training.csv', header=None)
class kamitani_data_handler():
    ...
    def get_voxel_loc(self):
        ...
        #x = ((x - x.min())/3).astype(int)
        #y = ((y - y.min())/3).astype(int)
        #z = ((z - z.min())/3).astype(int)
